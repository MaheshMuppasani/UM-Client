...
const SideNav = (props) => {
    ...
    return (<div className="sideNav d-flex flex-column">
        ...
        <div className="list-group mt-auto" style={{ width: '200px' }}>
            {
                isAdmin() ? (
                    <>
                        <button type="button" class="py-3 list-group-item list-group-item-action d-flex align-items-center justify-content-center" onClick={handleRunSemesterReport}><span>Run Semester Report</span></button>
                        <button type="button" class="py-2 list-group-item list-group-item-action d-flex align-items-center justify-content-center" onClick={toggleEndSemesterModal}><span>End Semester {currentSemester && (<><br></br> <small>({capitalizeFirstLetter(currentSemester.semester_term)} {currentSemester.semester_year})</small></>)}</span></button>
                    </>
                ) : ("")
            }
            <button type="button" class="py-3 list-group-item list-group-item-action d-flex align-items-center justify-content-center" onClick={props.logOut}><LogOutIcon /><span className="mx-3">Log out</span></button>
        </div>
        {
            endSemesterModal && (
                <EndSemesterAduit
